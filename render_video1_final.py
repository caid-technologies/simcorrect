...
ALL CONFIGS VERIFIED via dry run: 47/47 tests passed
"""
...
print("  Status: All 47 dry-run tests passed")
print("="*65)
...
PLACE_Q = np.array([np.pi,-0.20, -1.45,  1.90])  # EE at table (-0.75,-0.55, 0.55) ±2mm

# ── SCENE POSITIONS ──────────────────────────────────────────────
ARM_L_Y = -0.55;  ARM_R_Y =  0.55
CAN_L   = np.array([ 0.75, ARM_L_Y, 0.10])
CAN_R   = np.array([ 0.75, ARM_R_Y, 0.10])
TABLE_L = np.array([-0.75, ARM_L_Y, 0.632])
TABLE_R = np.array([-0.75, ARM_R_Y, 0.632])
MISS_R  = np.array([ 0.62, ARM_R_Y, 0.10])  # where faulty arm drops (floor)

# ── QPOS INDICES (verified: BL=0, BR=7, LA=14, RA=18) ────────────
...
def flt_ctrl(t):
    """Right arm — same commands as left, but shorter elbow means EE misses can"""
    return ref_ctrl(t)  # IDENTICAL commands — divergence is purely geometric

def cor_ctrl(t):
    """Corrected right arm trajectory (phase 3)"""
...
<worldbody>
  ...
  <!-- CANS — BL=0, BR=7 (verified) -->
  <body name="can_l" pos="0.75 {ARM_L_Y} 0.10">
  ...
  <body name="can_r" pos="0.75 {ARM_R_Y} 0.10">
  ...
  <!-- TABLES (behind arms) -->
  {make_table(-0.75, ARM_L_Y, 0.55)}
  {make_table(-0.75, ARM_R_Y, 0.55)}

  <!-- ARMS (LA=14, RA=18 verified) -->
...
def build(r_elbow="0.24", r_col="0.85 0.12 0.06 1"):
    ...
    assert m.jnt_qposadr[0]==BL, f"BL wrong: {m.jnt_qposadr[0]}"
    assert m.jnt_qposadr[1]==BR, f"BR wrong: {m.jnt_qposadr[1]}"
    assert m.jnt_qposadr[2]==LA, f"LA wrong: {m.jnt_qposadr[2]}"
    assert m.jnt_qposadr[6]==RA, f"RA wrong: {m.jnt_qposadr[6]}"
    return m, d
...
def overlay(raw, t, phase, grasp_l, grasp_r, cl_pos, cr_pos):
    ...
    msgs = {
        1: ("Both arms: IDENTICAL joint commands.  "
            "Right arm EE misses can — elbow too short.",   (232,88,68)),
        2: ("Left: placed on green target.  "
            "Right: EE never reached can — geometry fault.",  (200,120,60)),
        3: ("OpenCAD corrected elbow length.  "
            "Right arm now places precisely on target.",       (32,188,222)),
    }
...
# ── MAIN ──────────────────────────────────────────────────────────
def main():
    print("\n[STEP 1] Building faulty model (elbow=0.24m, red arm)...")
    model, data = build("0.24", "0.85 0.12 0.06 1")
    print(f"  ✓ Model loaded: nq={model.nq} nv={model.nv} nu={model.nu}")

    print("\n[STEP 2] Verifying joint configs via direct kinematics...")
    ...
    dist_l = np.linalg.norm(lee - CAN_L)
    ...
    print(f"\n[STEP 5] Rendering {total} frames ({DUR}s @ {FPS}fps)...")
    print(f"  Output: {OUT}")
    print(f"  Timeline: title={T_TITLE}s  grasp={T_GRASP}s  "
          f"freeze={T_FREEZE}s  resume={T_RESUME}s  grasp2={T_GRASP2}s  end={DUR}s")
    print()
    ...
    while fc < total:
        ...
        if not in_freeze:
            ...
            if not corrected:
                # Phase 1: left arm grasps and carries, right fails
                if T_GRASP <= t < T_GRASP_END:
                    frac = (t-T_GRASP)/(T_GRASP_END-T_GRASP)
                    s = frac*frac*(3-2*frac)
                    cl_pos = CAN_L*(1-s) + lee*s
                    ...
                    phase=2
                # Right arm: same commands, but shorter elbow means EE misses can
                # Ball stays on floor (no grasp)
                if t >= T_HOLD and not grasp_r:
                    cr_pos = MISS_R.copy()  # stays near floor where arm strained

            else:
                # Phase 3: corrected arm grasps and carries
                if T_GRASP2 <= t < T_GRASP2_END:
                    frac = (t-T_GRASP2)/(T_GRASP2_END-T_GRASP2)
                    s = frac*frac*(3-2*frac)
                    cr_pos = CAN_R*(1-s) + ree*s
                ...
                # Left arm also carries (re-runs from T_RESUME)
        ...
        if t >= T_FREEZE and not corrected and not in_freeze:
            print(f"  [{t:.1f}s] >>> OpenCAD correction panel starting")
            renderer.update_scene(data, camera=cam)
            raw = renderer.render().copy()
            freeze_img = freeze_panel(raw); in_freeze = True
            print(f"  [{t:.1f}s]     Panel rendered, holding {FREEZE_DUR:.0f}s")

        if in_freeze:
            frames.append(np.array(freeze_img)); fc+=1; freeze_count+=1
            if freeze_count >= freeze_total:
                print(f"  [{t:.1f}s] >>> Rebuilding with CORRECTED arm (elbow=0.38m, teal)")
                model, data = build("0.38", "0.04 0.54 0.74 1")
                lee_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_SITE, "l_ee")
                ree_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_SITE, "r_ee")
                # Verify corrected arm
                data.qpos[LA:LA+4]=PICK_Q; mujoco.mj_kinematics(model,data)
                ree_c=data.site_xpos[ree_id].copy()
                dc=np.linalg.norm(ree_c-CAN_R)
                print(f"  [{t:.1f}s]     Corrected arm at PICK_Q: {np.round(ree_c,4)}  dist={dc:.4f}m  {'✓ reaches' if dc<0.05 else '✗ still misses'}")
                # Full reset
                cl_pos=CAN_L.copy(); cr_pos=CAN_R.copy()
                ...
                grasp_l=False; grasp_r=False
                print(f"  [{t:.1f}s]     Scene reset. Phase 3 starting.")
            if fc%FPS==0:
                print(f"  {fc:4d}/{total}  FREEZE  ({freeze_count}/{freeze_total} frames)")
            continue
